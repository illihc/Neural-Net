...
const double NeuralNet::GetResult(unsigned NeuronCount)
{
	std::cout << "NeuralNet::GetResult: " << Layers.back()[NeuronCount].GetOutputValue() << " NeuronCount is: "<< NeuronCount<< std::endl;
	return Layers.back()[NeuronCount].GetOutputValue();
}
...
void TrainNet(unsigned _TrainingSteps)
{
	...
	while (MaxTrainingsSteps > CurrentTrainingStep)
	{
		...
		while (std::getline(InputValueFile, FileLine))
		{
			...
			//Squezzing the input between 0 and 1
...
double GetNetResult(unsigned ParameterCount)
{
	std::cout << "GetNetResult: " << Net.GetResult(ParameterCount) << " NeuronCount is: " << ParameterCount << std::endl;
	return Net.GetResult(ParameterCount);
}
